...
assert buildLogText.contains("Executing: pt-online-schema-change --alter-foreign-keys-method=auto --nocheck-unique-key-change --alter=\"ADD CONSTRAINT uq_id_name UNIQUE (id, name)\" --host=127.0.0.1 --port=${config_port} --user=${config_user} --password=*** --execute D=${config_dbname},t=person");
...
assert buildLogText.contains("Unique constraint added to person(id, name)");
