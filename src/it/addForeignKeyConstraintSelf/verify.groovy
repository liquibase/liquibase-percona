...
assert buildLogText.contains("Executing: pt-online-schema-change --alter-foreign-keys-method=auto --nocheck-unique-key-change --alter=\"ADD CONSTRAINT fk_person_parent FOREIGN KEY (parent) REFERENCES _person_new (id)\" --host=127.0.0.1 --port=${config_port} --user=${config_user} --password=*** --execute D=${config_dbname},t=person");
...
assert buildLogText.contains("Foreign key constraint added to person (parent)");
