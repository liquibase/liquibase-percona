...
assert buildLogText.contains("test-changelog.xml::2::Alice: Executing: pt-online-schema-change --alter=\"ADD CONSTRAINT fk_person_parent FOREIGN KEY (parent) REFERENCES _person_new (id)\" --alter-foreign-keys-method=auto --host=127.0.0.1 --port=${config_port} --user=${config_user} --password=*** --execute D=${config_dbname},t=person");
