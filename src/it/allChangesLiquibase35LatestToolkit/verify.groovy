...
assert sqlText.contains("${perconaFullPath} --alter=\"ADD COLUMN age INT NULL\"")
assert sqlText.contains("${perconaFullPath} --alter=\"DROP COLUMN age\"")
assert sqlText.contains("${perconaFullPath} --alter=\"ADD UNIQUE INDEX emailIdx (email)\"")
assert sqlText.contains("${perconaFullPath} --alter=\"DROP INDEX emailIdx\"")
assert sqlText.contains("${perconaFullPath} --alter=\"MODIFY email VARCHAR(400)\"")
assert sqlText.contains("${perconaFullPath} --alter=\"ADD CONSTRAINT fk_person_address FOREIGN KEY (person_id) REFERENCES person (name)\"")
assert sqlText.contains("${perconaFullPath} --alter=\"DROP FOREIGN KEY _fk_person_address\"")
