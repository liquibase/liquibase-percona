...
assert buildLogText.contains("Executing: pt-online-schema-change --alter-foreign-keys-method=auto --no-check-unique-key-change --no-check-alter --alter=\"DROP PRIMARY KEY, ADD PRIMARY KEY (id, name)\" --host=127.0.0.1 --port=${config_port} --user=${config_user} --password=*** --execute D=${config_dbname},t=person")
...
assert buildLogText.contains("Primary key added to person (id, name)")
...
try {
    ...
    r = s.executeQuery("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='${config_dbname}' AND TABLE_NAME='person' AND COLUMN_KEY='PRI' ORDER BY 1");
    assert r.first()
    assert r.getString(1) == "id"
    assert r.next()
    assert r.getString(1) == "name"
    assert r.isLast()
} finally {
    s?.close();
    con?.close();
